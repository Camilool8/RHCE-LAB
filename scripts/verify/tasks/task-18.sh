...
task_verify() {
    ...
    # Bonus: was ansible-navigator actually used for selinux2.yml?
    if [[ -f /tmp/verify-apply-selinux2-nav.log ]]; then
        SCORE_POINTS=$((SCORE_POINTS + 1)); SCORE_MAX=$((SCORE_MAX + 1))
        SCORE_DETAIL+=("${C_GRN}PASS${C_OFF} (+1) ansible-navigator was used for selinux2.yml")
    else
        SCORE_MAX=$((SCORE_MAX + 1))
        SCORE_DETAIL+=("${C_YEL}WARN${C_OFF} ( 0/1) selinux2.yml not detected as run via ansible-navigator")
    fi
}

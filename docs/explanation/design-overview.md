...
- A package source so package-install tasks work without internet.
...
6. **Role-specific** — exactly one of:
   - **`scripts/repo-server/*`** on `repo`: HTTP repos, NFS exports, GPG key.
   - **`scripts/control/setup-control.sh`** on `control`: ansible-core +
     navigator + EE image + populated `/etc/hosts`.
   - **`scripts/node/setup-node.sh`** on `node*`: SSH key authorization,
     `research` VG on the second extra disk, NFS automount fstab.
...
exists for the `research` VG. The student's playbooks should do the same.
